...
-- Functions
--
CREATE FUNCTION `canonicalise route` (`route` VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci  return regexp_replace(regexp_replace(route, '(^|[^0-9])([0-9]{1,2})([^0-9]|$)', '\\10\\2\\3'), '(^|[^0-9])([0-9]{1,2})([^0-9]|$)', '\\10\\2\\3')$$

CREATE FUNCTION `get tickets count` (`journey serial` BIGINT UNSIGNED) RETURNS INT(10) UNSIGNED DETERMINISTIC return (select count(0) from `ticket uses` where `ticket uses`.`journey serial` = `journey serial`)$$

CREATE FUNCTION `is fully ticketed` (`journey serial` BIGINT UNSIGNED) RETURNS TINYINT(1)  return (
