...
alter view `tickets view` as
    select serial
        ...
        , `segments travelled`
        , `distance travelled`
        , if(expired, price, price / carnets * `carnets used`) as `price used`
        , if(expired, price, price / carnets * `carnets used`) / `distance travelled` as `price per km`
    from tickets join (
        select serial as `_serial`
        , (
            select max(`group size`)
            from `ticket uses` join journeys on `journey serial` = serial
            where `ticket serial` = tickets.serial
        ) as `maximum group size`
        , (
        ...
        , (
            select ifnull(sum(ifnull(`distance covered`, distance) * `group size`),0)
            from `ticket uses` join journeys on `journey serial` = serial
            where `ticket serial` = tickets.serial
